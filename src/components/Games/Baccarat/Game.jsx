...
const CardBack = ({ rand, top = "50%" }) => (
  <div className={`w-24 h-36 card card${rand} relative rounded-md shadow-lg bg-blue-600 border-2 border-white flex items-center justify-center`}>
    <div className="absolute top-0 left-0 w-full h-full bg-black/30 z-10"></div>
    <h1 className={`text-white font-medium absolute top-[${top}] left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 text-[0.9rem]`}>
      Khel <br />
...
const Game = ({ cardsNumber = 6,
  ...
  const placeBet = (type) => {
    if (betStarted ) return; // Prevents betting if game hasn't started or bet is 0
      ...
      if (type === "Player") setPlayerBet((prev) => prev + (Number(chipBet) || 0));
      if (type === "Tie")setTieBet((prev) => prev + (Number(chipBet) || 0));
    if (type === "Banker") setBankerBet((prev) => prev + (Number(chipBet) || 0));
  };
  ...
  useEffect(() => {
    if (gameStarted) {
      ...
      if (deck.length > 0 && userCards.length + bankerCards.length < cardsNumber) {
  ...
  useEffect(() => {
  if (userCards.length === cardsNumber / 2 && bankerCards.length === cardsNumber / 2) {
    setTimeout(() => {
      const winnerText =
  ...
  return (
    <div className="relative w-full h-[600px] text-base text-white overflow-hidden">
      <div className="w-full h-[450px] relative">
        ...
        <div className="absolute right-28 top-[-85px] z-10">
          {deck.map((card, i) => (
            <motion.div
              ...
              className="absolute"
              style={{ zIndex: 10 - i, transform: `rotate(${i % 2 === 0 ? i : -i}deg)` }}
            >
        ...
        <div className="absolute px-4 py-2 rounded bg-gray-800 top-10 left-[30%]">Player: {playerScore}</div>
        <div className="absolute px-4 py-2 rounded bg-gray-800 top-10 left-[55%]">Banker: {bankerScore}</div>

        {/* Render Player Cards */}
        {userCards.map((card, index) => (
          <FlippableCard key={card.id} card={card} position={{ top: 35 + index * 6, left: 20 + index * 4 }} />
        ))}

        {/* Render banker Cards */}
        {bankerCards.map((card, index) => (
          <FlippableCard key={card.id} card={card} position={{ top: 35 + index * 6, left: 57 + index * 4 }} />
        ))}
...
{bankerCards.map((card, index) => (
  ...
  <motion.div
    ...
    className={`absolute top-1/2 left-[45%] transform -translate-x-1/2 -translate-y-1/2 px-4 py-2 rounded-lg text-white text-sm font-bold 
      ${winner === "Player Wins!" ? "border-2 border-green-500 bg-green-900/80" : ""}
      ${winner === "It's a Tie!" ? "border-2 border-blue-500 bg-blue-900/80" : ""}
      ${winner === "Banker Wins!" ? "border-2 border-red-500 bg-red-900/80" : ""}`}
  >
    {winner === "Player Wins!" && (
      <div className="flex flex-col items-center">
        <span className="text-lg">2x</span>
        <span>${(playerBet * 2).toFixed(2)}</span> {/* Double Player Bet */}
      </div>
    )}
    ...
    {winner === "It's a Tie!" && (
      <div className="flex flex-col items-center">
        ...
        <span>${(tieBet * 8).toFixed(2)}</span> {/* Multiply Tie Bet by 8 */}
      </div>
    )}
    ...
    {winner === "Banker Wins!" && (
      <div className="flex flex-col items-center">
        ...
        <span>${(bankerBet * 1.95).toFixed(2)}</span> {/* Deduct 5% Commission */}
      </div>
    )}
  </motion.div>
        )}
